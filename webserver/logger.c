...
{				/* alloca la memoria per un logNode */
...
struct logNode *create_logNode(int type)
{				/* crea un logNode con messaggio uguale alla stringa text */
...
{				/* alloca la memoria per un logger */
...
void destroy_logNode(struct logNode *ln)
{				/* libera la memoria di un logNode */
...
{				/* crea un logger che scrive nel file di log del percorso indicato da pathLog */
...
void insert_logNode(struct logger *myLogger, struct logNode *ln)
{				/* inserisce un nuovo logNode nel logger */
...
void delete_logNode(struct logNode *ln)
{				/* elimina un logNode dal logger */
...
void summary_logger(struct logger *myLogger)
{				/*stampa il testo di tutti i logNode presenti nel logger */
...
void write_on_file(struct logger *myLogger, struct logNode *ln)
{
	...
	char messageType[5];
	if ((ln->type) == ERR)
		strcpy(messageType, "ERR");
	else if ((ln->type) == WRN)
		strcpy(messageType, "WRN");
	else
		strcpy(messageType, "NFO");
...
void *garbadge(void *p)
{
	...
	while (1) {
		...
		if (is_logger_empty(myLogger))
			wait_for_message(myLogger);
